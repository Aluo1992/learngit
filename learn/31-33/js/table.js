...
function render(arr1,arr2){
    ...
    if(r.length==1&&p.length==1){
       ...
       dataList[0].sale.map(x=>{saleStr=saleStr+'<td>'+x+'</td>';});
    ...
    if(r.length==1&&p.length>1){
        ...
        dataList[0].sale.map(x=>{c=c+'<td>'+x+'</td>'});
        c=c+'</tr>';
        for(let i=1;i<dataList.length;i++){
            c=c+'<tr><td>'+dataList[i].product+'</td>';
            dataList[i].sale.map(x=>{c=c+'<td>'+x+'</td>';});
        }
        c=c+'</tr>';
        table.innerHTML=c;
    }
    ...
    if(r.length>1&&p.length>1){
        ...
        for(let i=0;i<p.length;i++){
            c=c+'<tr><td rowspan=\"'+r.length+'\">'+p[i]+'</td><td>'+r[i]+'</td>';
            dataList.filter(x=>x.region==r[i]&&x.product==p[i])[0].sale.map(x=>{c=c+'<td>'+x+'</td>'});
            c=c+'</tr>';
            for(let j=1;j<r.length;j++){
               c=c+'<tr><td>'+r[j]+'</td>';
               dataList.filter(x=>x.region==r[j]&&x.product==p[i])[0].sale.map(x=>{c=c+'<td>'+x+'</td>'});
               c=c+'</tr>';
            }
        }
        table.innerHTML=c;
